...
class Tour
{
    ...
    */
    ...
    function __construct($tour_id) {
        ...
        // Not done for a FFA tournamnent
        if ($this->empty && $this->type != TT_SINGLE) {
            $this->delete();
        }
    ...
    public static function getToursByType($type) {

        /**
         * Returns an array of all tournament objects of the given type
         **/

        $tours = array();
        $result = mysql_query("SELECT tour_id FROM tours WHERE type=" .$type. " ORDER BY date_created DESC");
        if (mysql_num_rows($result) > 0) {    
            while ($row = mysql_fetch_assoc($result)) {
                array_push($tours, new Tour($row['tour_id']));
            }
    ...
    public static function create(array $input) {
         ...
         * Arguments:
         * ----------
         * 	did, name, type, rs, teams => array(team_ids, ...), 'rounds' (ONLY if round-robin or single matches, else ignored)
         **/
        ...
        // Not single match and team array OK?
