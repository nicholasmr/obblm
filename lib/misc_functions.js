...
/*************************
 *  These functions are used in the administrator's section by the "schedule matches" sub-section.
 *************************/
...
function chTour(t)
{
    ...
    */
    ...
    et = document.getElementById('existTour');
...
/*************************
 *  These functions are used in the records section.
 *************************/

function updateTeamPlayers(tid, psel) /* Team ID and player select object. */
{
    psel.options.length = 0; /* Clear */
    for (p in jsteams[tid]) { /* p is numerical from [0; n-1] players */
        psel.options[p] = new Option(jsteams[tid][p]['name'], jsteams[tid][p]['pid']);
    }
    return true;
}
