...
public function getGoods($double_RRs = false)
{
    ...
    return array(
            ...
            'apothecary'    => array('cost' => $rules['cost_apothecary'],   'max' => ($apoth ? 1 : 0),              'item' => 'Medico'),
            'rerolls'       => array('cost' => $rr_price,                   'max' => $rules['max_rerolls'],         'item' => 'Segunda Oportunidad'),
            'ff_bought'     => array('cost' => $rules['cost_fan_factor'],   'max' => $rules['max_fan_factor'],      'item' => 'Factor de Hinchas'),
            'ass_coaches'   => array('cost' => $rules['cost_ass_coaches'],  'max' => $rules['max_ass_coaches'],     'item' => 'Asistente de Entrenador'),
            'cheerleaders'  => array('cost' => $rules['cost_cheerleaders'], 'max' => $rules['max_cheerleaders'],    'item' => 'Animadora'),
    );
}
