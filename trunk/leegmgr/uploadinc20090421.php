...
#	$i_away = $index['REPORT'][2];
	...
	#Print "<b>Away Player Indexes</b><br>";
	...
	while ( $i < ($countihomerplayers-1) ) {
		...
		#Print "<br>This is i: ".$i;
		#Print "<br>This is count of i homeplayers: ".(count($i_homeplayers)-1);
		if ( $vals[$i][attributes][PLAYER] > 0 ) {

			$homeplayers[$h][number] = $vals[$i][attributes][PLAYER];
			#Print "<b>Player Number: ".$homeplayers[$h][number]."</b>";
	...
	while ( $i < ($countiawayrplayers-1) ) {
		...
		if ( $vals[$i][attributes][PLAYER] > 0 ) {
			...
			#Print "<b>Player Number: ".$awayplayers[$h][number]."</b>";
	...
	#Print "<br><br><br><br><br>".$hash."<br><br><br><br><br>";
	...
	#Print "<br><br><br><br>";
	#Print "<b>Gate: </b>".$gate."<br>";
	#Print "<b>Home Team</b>: ".$hometeam."<br>";
	#Print "Score: ".$homescore."<br>";
	#Print "Winnings: ".$homewinnings."<br>";
	#Print "Fan Factor Adj.: ".$homeff."<br>";
	#Print "<br>";
	#Print "<b>Away Team</b>: ".$awayteam."<br>";
	#Print "Score: ".$awayscore."<br>";
	#Print "Winnings: ".$awaywinnings."<br>";
	#Print "Fan Factor Adj.: ".$awayff."<br>";
...
function report ( $homeplayers, $awayplayers, $gate, $hometeam, $homescore, $homewinnings, $homeff, $awayteam, $awayscore, $awaywinnings, $awayff, $hash ) {
	...
	#Print "<br>TEAM_ID:".$awayteam_id;

	#DATE FORMAT  2009-02-24 22:11:17
	date_default_timezone_set('EST');
	$c_date = date ( "Y\-m\-d H\:i\:s" );
	#Print "<br><b>The hash is: </b>".$hash."<br>";
	...
	$query="INSERT INTO matches ( round, f_tour_id, locked, submitter_id, stadium, gate, ffactor1, ffactor2, income1, income2, team1_id, team2_id, date_created, date_played, date_modified, team1_score, team2_score, hash)
		VALUES ( 255, 1, 1, 1, $hometeam_id, $gate, $homeff, $awayff, $homewinnings, $awaywinnings, $hometeam_id, $awayteam_id, '$c_date', '$c_date', '$c_date', $homescore, $awayscore, '$hash' )";
	...
	if (!$result) {
		Print "<br>The unique match id could not be found.<br>";
		die('BEGIN REPORT MATCH_DATA TABLE Query 1 failed: ' . mysql_error());
		exit(-1);
	}
	$result = mysql_fetch_array($result);
	$f_team1_id = $result['team1_id'];
	$f_match_id = $result['match_id'];
	$f_tour_id = $result['f_tour_id'];

	$query = "SELECT owned_by_coach_id FROM `teams` WHERE team_id= \"".$f_team1_id."\"";
	$result = NULL;
	$result = mysql_query($query);
	if (!$result) {
	...
	while ( $i < count( $homeplayers ) ) {
		Print $homeplayer_nr = $homeplayers[$i][number];
		$query = "SELECT player_id FROM `players` WHERE date_sold is NULL and owned_by_team_id = ".$f_team1_id." AND nr = " .$homeplayer_nr;
		$result = NULL;
		$result = mysql_query($query);
		if (!$result) {
		...
		$query = "INSERT INTO match_data ( f_coach_id, f_team_id, f_player_id, f_match_id, f_tour_id, mvp, cp, td, intcpt, bh, si, ki, inj, agn1, agn2 )
			VALUES ( $f_coach_id, $f_team1_id, $f_player_id, $f_match_id, $f_tour_id, $mvp, $cp, $td, $intcpt, $bh, 0, 0, $inj, 1, 1 )";
		$result = NULL;
		$result = mysql_query($query);  #, $mysql_link
		if (!$result) {
	...
	$query = "SELECT player_id FROM `players` WHERE date_sold is NULL and owned_by_team_id = ".$f_team1_id;
	$result = NULL;
	$result = mysql_query($query);
	if (!$result) {
	...
	while ( $row = mysql_fetch_array($result) )
	{
		...
		$inj = 1;
		if ( !mysql_fetch_array(mysql_query("SELECT f_match_id  FROM `match_data` WHERE f_player_id  = ".$f_player_id." and f_match_id = ".$f_match_id)) )
		{
			$query = "INSERT INTO match_data ( f_coach_id, f_team_id, f_player_id, f_match_id, f_tour_id, mvp, cp, td, intcpt, bh, si, ki, inj, agn1, agn2 )
				VALUES ( $f_coach_id, $f_team1_id, $f_player_id, $f_match_id, $f_tour_id, $mvp, $cp, $td, $intcpt, $bh, 0, 0, $inj, 1, 1 )";
...
#	$result = mysql_query($query);
#	if (!$result) {
#		die('BEGIN REPORT MATCH_DATA TABLE Query WINNINGS failed: ' . mysql_error());
#		exit(-1);
#	}
#	$result = mysql_fetch_array($result);
#	$c_treasury = $result['treasury'];
#
#	$u_treasury = $c_treasury + $homewinnings;
#
#	$query = "UPDATE teams SET treasury = ".$u_treasury." WHERE team_id = ".$team_id;
#	$result = NULL;
#	$result = mysql_query($query);  #, $mysql_link
#	if (!$result) {
#		die('BEGIN REPORT MATCH_DATA TABLE UPDATE WINNINGS failed: ' . mysql_error());
#		exit(-1);
#	}
	updateWinnings ( $f_team1_id, $homewinnings );
	...
	$result = mysql_fetch_array($result);
	$f_team2_id = $result['team2_id'];
	$f_match_id = $result['match_id'];
	$f_tour_id = $result['f_tour_id'];

	$query = "SELECT owned_by_coach_id FROM `teams` WHERE team_id= \"".$f_team2_id."\"";
	$result = NULL;
	$result = mysql_query($query);
	if (!$result) {
	...
	while ( $i < count( $awayplayers ) ) {
		Print $awayplayer_nr = $awayplayers[$i][number];
		$query = "SELECT player_id FROM `players` WHERE date_sold is NULL and owned_by_team_id = ".$f_team2_id." AND nr = " .$awayplayer_nr;
		$result = NULL;
		$result = mysql_query($query);
		if (!$result) {
		...
		$query = "INSERT INTO match_data ( f_coach_id, f_team_id, f_player_id, f_match_id, f_tour_id, mvp, cp, td, intcpt, bh, si, ki, inj, agn1, agn2 )
			VALUES ( $f_coach_id, $f_team2_id, $f_player_id, $f_match_id, $f_tour_id, $mvp, $cp, $td, $intcpt, $bh, 0, 0, $inj, 1, 1 )";
		$result = NULL;
		$result = mysql_query($query);  #, $mysql_link
		if (!$result) {
	...
	$query = "SELECT player_id FROM `players` WHERE date_sold is NULL and owned_by_team_id = ".$f_team2_id;
	$result = NULL;
	$result = mysql_query($query);
	if (!$result) {
		die('****Query ln 421 failed: ' . mysql_error());
...
#	$result = mysql_query($query);
#	if (!$result) {
#		die('BEGIN REPORT MATCH_DATA TABLE Query AWAY WINNINGS failed: ' . mysql_error());
#		exit(-1);
#	}
#	$result = mysql_fetch_array($result);
#	$c_treasury = $result['treasury'];
#
#	$u_treasury = $c_treasury + $awaywinnings;
#
#	$query = "UPDATE teams SET treasury = ".$u_treasury." WHERE team_id = ".$team_id;
#	$result = NULL;
#	$result = mysql_query($query);  #, $mysql_link
...
#		exit(-1);
#	}
	updateWinnings ( $f_team2_id, $awaywinnings );
	#END AWAY WINNINGS

	
#	$i = 0;
#	Print count( $homeplayers );
#	while ( $i < count( $homeplayers ) ) {
#		$query="INSERT INTO matches (match_id, round, f_tour_id, locked, submitter_id, stadium, gate, ffactor1, ffactor2, income1, income2, team1_id, team2_id, date_created, date_played, date_modified, team1_score, team2_score)
#			VALUES ( , , , 1, , , , , , , , , , , , , , )"; 
